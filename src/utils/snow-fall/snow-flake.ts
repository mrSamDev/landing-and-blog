...
class Snowflake {
    ...
    public updateConfig(config: SnowflakeConfig): void {
        ...
        if (this.params && JSON.stringify(this.config.radius) !== JSON.stringify(previousConfig?.radius)) {
            this.params.radius = random(...this.config.radius);
        }

        if (JSON.stringify(this.config.images) !== JSON.stringify(previousConfig?.images)) {
            this.selectImage();
        }

        if (previousConfig?.opacity && JSON.stringify(this.config.opacity) !== JSON.stringify(previousConfig?.opacity)) {
