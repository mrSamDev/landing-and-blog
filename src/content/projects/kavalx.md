---
title: Kavalx
description: AI PR Review Agent for Bitbucket and self-hosted teams. Controllable, self-hostable code review with retrieval, memory, and architecture-aware agent orchestration.
...
Existing tools assume GitHub primitives. Checks API, GraphQL, Actions. Self-hosted teams need transparent, auditable review workflows. Tool access to source code must be controllable and offline-capable. Enterprise review policies are not one-size-fits-all.
...
- TypeScript
- Bitbucket Server and Data Center APIs
- Vector DB for semantic retrieval
- Ollama, OpenAI, Anthropic (provider-agnostic model layer)

## Current Status

Closed source. The core engine is not public yet. Beta users. First enterprise teams are running it against real PR volumes. A small group of engineers are contributing time. No paid development at this stage.

It will go OSS once the agent orchestration, memory system, and eval pipeline are stable enough that external contributors can run it without debugging our assumptions.
